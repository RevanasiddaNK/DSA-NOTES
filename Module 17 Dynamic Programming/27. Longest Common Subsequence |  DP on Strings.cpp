...
int solveRec(int i1, int i2, string s1, string s2){
    ...
    if(s1[i1] == s2[i2]){
        ...
        return 0+max(solveRec(i1-1,i2,s1,s2), solveRec(i1,i2-1,s1,s2));
    }
}

//Memoization
// Shifting of INDEXes method
int solveMem(int i1, int i2, string s1, string s2, vector<vector<int>>&dp){

    if(i1 == 0 || i2 == 0)  return 0;

/*
// it is an correct base case but we are modifying the base case for right shifting of index
    if(i1 < 0 || i2 < 0)
        return 0;
*/
/*
this is wrong bcz when i1 = 0 && i2 == 8 every element is s2 is not compared with s1[0]
    if(i1 == 0 || i2 == 0)
        return s1[i1] == s2[i2];
*/
    if(dp[i1][i2] != -1)
    ...
    if(s1[i1-1] == s2[i2-1]){
        ...
        return dp[i1][i2] = 0+max(solveMem(i1-1,i2,s1,s2,dp), solveMem(i1,i2-1,s1,s2,dp));
    }
}
