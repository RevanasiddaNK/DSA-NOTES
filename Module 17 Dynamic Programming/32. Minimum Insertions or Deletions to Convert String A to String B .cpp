...
int MinimumInsertionsDeletionstoConvertStringAtoStringB(string &s1, string &s2){
    
    int len1 = s1.size();
    int len2 = s2.size();
//let consider we are changing s2 to s1 
    int deletions = len2 - LongestCommonSubsequence(s1,s2);
    int insertions = len1 - LongestCommonSubsequence(s1,s2);
...
int canYouMake(string &s1, string &s2){
    ...
    MinimumInsertionsDeletionstoConvertStringAtoStringB(s1,s2);
}
